...
class text {
	...
	public static function getText($keyname) {
        ...
        if(isset($_GET['edit'])) {
            ...
            $row = $db->exec_SELECTgetRows("uid,t_content_de,t_content_en,t_content_fr,t_content_it,t_description","text","t_keyname = '".$keyname."'");
            //echo $db->view_array($row);
            if(strlen($row[0]['t_content_de'])>50) {
                $text.='<textarea name="'.$row[0]['uid'].'" value"">'.htmlentities($row[0][$lang_col]?$row[0][$lang_col]:$row[0]['t_content_de']).'</textarea><br>';
            } else {
                $text.='<input type="text" name="'.$row[0]['uid'].'" value="'.htmlentities($row[0][$lang_col]?$row[0][$lang_col]:$row[0]['t_content_de']).'"><br>';
            }
    ...
    public static function updateValues() {
        ...
        foreach ($_POST as $key => $val) {
            if($key!='submit') {
                $db->exec_UPDATEquery('text',"uid=".$key,array($lang_col=>$val));
                //echo($key.":::".$val."<br>"); 
                
                //echo htmlspecialchars($db->debug_lastBuiltQuery);
            }
        }
        return $succ;    
    }
    ...
    public static function saveText() {
        ...
        foreach($_POST as $key=>$text) {
             $db->exec_UPDATEquery('text','t_keyname=\''.$key.'\'',array('t_content_'.$lang=>$text));
             echo($key.":::".$text."<br>");
        }
    }
