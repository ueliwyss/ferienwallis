	...
	if($view=='list') {
		...
		if($action['mode']!=table::MODE_NEW) {
			...
			if(user::curusr_getPermission("usr_user_perms")>=user::PERMISSION_FULL) {
				...
				$regNewPerm=new formButton('Berechtigung hinzufügen',"top.editElement('".table::MODE_NEW."','usr_userhasperm','".NO_UID."','".$action['uid']."',true)",ROOT_DIR.'icons/ilist_actions_add.gif');
			...
			if(user::curusr_getPermission("usr_user_groups")>=user::PERMISSION_FULL) {
				...
				$regNewGroup=new formButton('Gruppe hinzufügen',"top.editElement('".table::MODE_NEW."','usr_useringroup','".NO_UID."','".$action['uid']."',true)",ROOT_DIR.'icons/ilist_actions_add.gif');
