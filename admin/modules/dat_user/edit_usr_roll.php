...
if($view=='list') {
	...
	if($action['mode']!=table::MODE_NEW) {
		...
		if(user::curusr_getPermission("usr_roll_perms")>=user::PERMISSION_FULL) {
			...
			$regNewPerm=new formButton('Berechtigung hinzufügen',"top.editElement('".table::MODE_NEW."','usr_rollhasperm','".NO_UID."','".$action['uid']."',true)",ROOT_DIR.'icons/ilist_actions_add.gif');
