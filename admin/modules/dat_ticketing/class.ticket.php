...
$TABLES['ticket']=Array(
	...
	'lists'=>Array(
		...
		'pool_1-Level'=>Array(
			'title'=>'Ticket-Pool',
			'description'=>'Diese Auflistung zeigt diejenigen Tickets, welche noch keinen Besitzer haben (noch keinem Agent zugewiesen wurden), aber ihrem Support-Team zugewiesen wurden. Durchs klicken auf den Annehmen-Button (<img src="icons/ilist_actions_assume.gif">) wird das Ticket Ihnen zugewiesen.',
			'fields'=>'t_number,t_trigger_id,t_priority,t_description,t_status',
			...
			'actions'=>array(
				...
				'assume'=>array(
					...
					'icon'=>'icons/ilist_actions_assume.gif',
...
class ticket {
	...
	function getNextSupportLevelButton($ticket_id) {
		...
		if($nextSupportLevel) {
			$button=new formButton("An Support-Level ".$nextSupportLevel." weiterleiten","javascript:top.editElement('edit','ticket','".NO_UID."','".$ticket_id."',true);","icons/ilist_actions_assignTeam.gif");
		}
		return $button;
	}
	...
	function getAssignToTeamUserButton($ticket_id) {
		$button=new formButton("Zuweisen innerhalb Team","javascript:top.editElement('new','usr_user','".NO_UID."','".$ticket_id."',true,'','','ofTeam');","icons/ilist_actions_assignUser.gif");
