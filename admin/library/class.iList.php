...
.ilist_description {
	...
	public function wrapFooter() {
		...
		$content='
<tr>
<td colspan="'.$this->getNumCols(true).'" style="text-align:center;">
...
<td class="ilist_nav_pages"><nobr>';
	...
	public function prepQuery($force=false) {
		...
		if(!$this->queryGenerated || $force) {
			...
			$this->query['orderBy']=($this->query['orderBy']?" ORDER BY ".$this->query['orderBy']:"");
...
function multiDelete(listNum,tableName) {
	...
	public function getBgColor() {
		static $colorInd;
		if($colorInd==(count($this->bgColor)-1)) {
			$colorInd=0;
		} else {
			$colorInd++;
		}
...
<td colspan="'.count($this->columns).'" style="height:30px">&nbsp;</td>';
	...
	public function hasCommonActions() {
		return is_array($this->commonActions);
	}
	...
	public function wrapCommonActions() {
		...
		foreach($this->commonActions as $name=>$action) {
			switch ($name) {
				case 'drop':
		...
		global $db;
			...
			if($this->hasActions()) {
				foreach($this->actions as $name=>$action) {
					...
					switch ($name) {
						case 'drop':
							$content.='<img src="'.$this->icons["ACTIONS_DROP"].'" style="cursor:pointer;" onClick="'.$action.'" alt="Löschen">';
							break;
						case 'edit':
							$content.='<img src="'.$this->icons["ACTIONS_EDIT"].'" style="cursor:pointer;" onClick="'.$action.'" alt="Eigenschaften editieren">';
							break;
						case 'add':
							$content.='<img src="'.$this->icons["ACTIONS_ADD"].'" style="cursor:pointer;" onClick="'.$action.'" alt="Zuordnen">';
							break;
						default:
							if(is_array($action)) {
	...
	function wrapDescription() {
		if($this->showDescription) {
			$content='
<tr>
	<td class="ilist_description" colspan="'.$this->getNumCols().'">
		'.$this->description.'
	</td>
</tr>';
		}
	...
	public function wrapCaption() {
		if($this->showCaption) {
			$content='
<tr>
	<td class="ilist_caption" colspan="'.$this->getNumCols().'">
		'.$this->caption.'
	</td>
</tr>';
		}
	...
	public function wrapContent() {
		...
		if(is_array($this->items)) {
			...
			for($i=0;$i<$numRows;$i++) {
				...
				$content['main'].='<tr bgcolor="'.$bgColor.'">
';
...
';
	...
	private function wrapItem($index) {
		$content.='<td>
<table class="ilist" border="1" cellpadding="0" cellspacing="0">
';
...
.ilist_connector {
	...
	private function wrapItem($index) {
		...
		$content .= '<tr id="'.self::getInstances().'_item_'.$index.'" bgcolor="'.$bgColor.'" oldColor="'.$bgColor.'">
	';
