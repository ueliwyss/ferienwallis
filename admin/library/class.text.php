	...
	public static function getText($keyname) {
        ...
        if(isset($_GET['edit'])) {
            ...
            if(strlen($row[0]['t_content_de'])>50) {
                $text.='<textarea name="'.$keyname.'" value"">'.htmlentities($row[0][$lang_col]?$row[0][$lang_col]:$row[0]['t_content_de']).'</textarea><br>';
            } else {
                $text.='<input type="text" name="'.$keyname.'" value="'.htmlentities($row[0][$lang_col]?$row[0][$lang_col]:$row[0]['t_content_de']).'"><br>';
            }
