...
class div {
    ...
    function date_getLastWeekday($timestamp,$weekday,$sameDayJump=false) {
        $weekdays=array(
            'Sunday'=>0,
            'Monday'=>1,
            'Tuesday'=>2,
        ...
        if(is_string($weekday)) $weekday=$weekdays[$weekday];
        
        $tmp=getdate($timestamp);
        
        if($sameDayJump AND $weekday==$tmp['wday']) $days_back=7;
        else $days_back=$tmp["wday"]>=$weekday?($tmp['wday']-$weekday):$tmp["wday"]+1+(6-$weekday);
        //echo '<br>DAYS:'.$days_back.' WDAY:'.$tmp["wday"].'<br>';
        
        return div::date_subtractDays($timestamp,$days_back);
    }
    
    function date_getNextWeekday($timestamp,$weekday,$sameDayJump=false) {
        $weekdays=array(
            'Sunday'=>0,
            'Monday'=>1,
            'Tuesday'=>2,
        ...
        if($sameDayJump AND $weekday==$tmp['wday']) $days_forth=7;
        else $days_forth=$tmp["wday"]<=$weekday?($weekday-$tmp['wday']):$weekday+(7-$tmp["wday"]);
        
        return div::date_addDays($timestamp,$days_forth);    
    }
    
    function date_ostersonntag($year) {
	...
	static function date_addDays($date,$num) {
        ...
        $date_before=getdate($date);
        $date_after=getdate($date+$days);
		$addition=(($date_before['year']!=$date_after['year'])?0:($date_before['yday']+$num==$date_after['yday']?0:($date_before['yday']+$num<$date_after['yday']?-60*60:60*60)));
        //if($addition!=0) echo " YEARDAYbefore:".($date_before['yday']+$num)." YEARDAYAFTER:".$date_after["yday"]." YEARS:".$date_before['year']." ".$date_after["year"]." DATEbefore: ".date('d.m.Y',$date)." DATEAFTER:".date('d.m.Y',$date+$days)." NUMDAYS:".$num." ADDITION:".$addition."<br>";
        $output=($date+$days+$addition);
		return $output;
	}
                
    static function date_subtractDays($timestamp,$num) {
        $days=(60*60*24*$num);
        $date_before=getdate($date);
        $date_after=getdate($date-$days);
        $addition=(($date_before['year']!=$date_after['year'])?0:($date_before['yday']-$num==$date_after['yday']?0:($date_before['yday']-$num<$date_after['yday']?-60*60:60*60)));
        //if($addition!=0) echo " YEARDAYbefore:".($date_before['yday']-$num)." YEARDAYAFTER:".$date_after["yday"]." YEARS:".$date_before['year']." ".$date_after["year"]." DATEbefore: ".date('d.m.Y',$date)." DATEAFTER:".date('d.m.Y',$date+$days)." NUMDAYS:".$num." ADDITION:".$addition."<br>";
        $output=($timestamp-$days+$addition);
        return $output;
    }
		...
		if(is_string($str)) {
			...
			If (preg_match_all("/<func>(.*?)<\/func>/U",$str,$function)) {
				for($i=0;$i<count($function[0]);$i++) {
					...
					eval("\$result=".$func.";");
					$str=preg_replace("/".preg_quote($function[0][$i],"/")."/s",$result,$str);
				}
			}
		}
		return $str;
