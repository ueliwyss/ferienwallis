...
class table {
	...
	static function getListColumns($listTableName,$list) {
		...
		$query = table::getListQuery($listTableName,$list);
		$fields=table::getFields($listTableName);
		$listConfig=table::getListPalette($listTableName,$list);
		$columns=array();

		$fieldList=explode(",",$listConfig['fields']);
		foreach($fieldList as $fieldName) {
			...
			$field=table::getField($tableName_ofField,$tmpFieldName);
			...
			$columns[$fieldName]=array(
				'local_table'=>$tableName_ofField,
				'dbField'=>$fieldName,
				'foreign_table'=>$field['foreign_table'],
				'foreign_key'=>$field['foreign_key'],
				'foreign_display'=>$field['foreign_display'],
				'isImage'=>$listConfig['isImage'],
				'imagePath'=>$listConfig['imagePath'],
			);
			if($tmp[0]) {
				$columns[$fieldName]['label']=table::getFieldLabel($tmp[0],$tmpFieldName);
			} else {
				if(table::getFieldLabel($listTableName,$tmpFieldName)) {
					$columns[$fieldName]['label']=table::getFieldLabel($listTableName,$tmpFieldName);
				} elseif(table::getFieldLabel($listConfig['mm_table'],$tmpFieldName)) {
					$columns[$fieldName]['label']=table::getFieldLabel($listConfig['mm_table'],$tmpFieldName);
				} elseif(table::getFieldLabel($listConfig['foreign_table'],$tmpFieldName)) {
					$columns[$fieldName]['label']=table::getFieldLabel($listConfig['foreign_table'],$tmpFieldName);
				} elseif(table::getFieldLabel($query['foreign_table'],$tmpFieldName)) {
					$columns[$fieldName]['label']=table::getFieldLabel($query['foreign_table'],$tmpFieldName);
				} elseif(table::getFieldLabel($query['mm_table'],$tmpFieldName)) {
					$columns[$fieldName]['label']=table::getFieldLabel($query['mm_table'],$tmpFieldName);
				} else {
					$tables=explode(",",$listConfig['local_table']);
					foreach($tables as $table) {
						if(table::getFieldLabel($table,$tmpFieldName)) {
							$columns[$fieldName]['label']=table::getFieldLabel($table,$tmpFieldName);
						}
					}
				}
			}
			...
			if($alloc=table::getAllocation($listTableName,$fieldName)) {
				$columns[$fieldName]['allocation']=$alloc;
			} elseif($alloc=table::getAllocation($listConfig['mm_table'],$fieldName)) {
				$columns[$fieldName]['allocation']=$alloc;
			} elseif($alloc=table::getAllocation($listConfig['foreign_table'],$fieldName)) {
				$columns[$fieldName]['allocation']=$alloc;
			}
		}
		return $columns;
...
static function editElement(action,tableName,uid,foreign_uid,popup,urlPref,urlExt) {
	...
	static function getListQuery($tableName,$listName) {
		...
		$query = array(
			'select'=>$listConfig['fields'],
			'local_table'=>$tableName.($listConfig['local_table']?",".$listConfig['local_table']:''),
			'mm_table'=>$listConfig['mm_table'],
			'mm_key'=>$listConfig['mm_key'],
			'mm_foreign_key'=>$listConfig['mm_foreign_key'],
			'foreign_table'=>$listConfig['foreign_table'],
			'whereClause'=>formElement::parseString($listConfig['whereClause']),
